...
class GPTNeoXAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.kv_bits = config.quant_config["kv_bits"]
        super().__init__()
        self.config = config
        ...
        self.q_proj = QuantLinear(config.hidden_size, config.hidden_size, bias=config.attention_bias, config=config)
        self.k_proj = QuantLinear(config.hidden_size, config.hidden_size, bias=config.attention_bias, config=config)
        self.v_proj = QuantLinear(config.hidden_size, config.hidden_size, bias=config.attention_bias, config=config)
        self.o_proj = QuantLinear(config.hidden_size, config.hidden_size, bias=config.attention_bias, config=config)
    ...
    def forward(
        ...
        key = self.key_quantizer.apply(key, self.kv_bits, True, 1)
        value = self.value_quantizer.apply(value, self.kv_bits, True, 1)
...
class GPTNeoXMLP(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        super().__init__()
        self.config = config
        self.dense_h_to_4h = QuantLinear(config.hidden_size, config.intermediate_size, config=config)
        self.dense_4h_to_h = QuantLinear(config.intermediate_size, config.hidden_size, config=config)
